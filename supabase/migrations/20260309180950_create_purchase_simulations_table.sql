/*
  ...
  1. New Tables
    - `purchase_simulations`
      ...
      - `is_favorite` (boolean) - Whether this simulation is favorited
      - `last_accessed_at` (timestamp) - Last time this simulation was accessed
  ...
  3. Indexes
    ...
    - Add index on last_accessed_at for recent access queries
*/
...
CREATE INDEX IF NOT EXISTS idx_purchase_simulations_last_accessed 
  ON purchase_simulations(last_accessed_at DESC);

CREATE TRIGGER update_purchase_simulations_updated_at
  BEFORE UPDATE ON purchase_simulations
  FOR EACH ROW
