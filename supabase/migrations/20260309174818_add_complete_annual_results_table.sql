/*
  # Create Complete Annual Results Table
  ...
  1. New Tables
    - `annual_results`
      ...
      - `final_average_weight_kg` (numeric) - Final live weight per head
      - `carcass_yield_percentage` (numeric) - Carcass yield percentage
      - `arroba_price` (numeric) - Price per arroba
      - `carcass_weight_kg` (numeric) - Calculated carcass weight
      - `arrobas_per_head` (numeric) - Arrobas per head
      - `total_arrobas` (numeric) - Total arrobas produced
      ...
      - `profit_per_head` (numeric) - Profit per head
      - `profit_margin_percentage` (numeric) - Profit margin as percentage
      - `final_result` (numeric) - Profit or loss amount
      - `cost_percentage` (numeric) - Costs as percentage of revenue
      - `expense_percentage` (numeric) - Expenses as percentage of revenue
      - `profit_percentage` (numeric) - Profit as percentage of revenue
  ...
  3. Important Notes
    ...
    - Total heads must be positive values
...
CREATE TABLE IF NOT EXISTS annual_results (
  ...
  final_average_weight_kg numeric NOT NULL DEFAULT 0 CHECK (final_average_weight_kg >= 0),
  carcass_yield_percentage numeric NOT NULL DEFAULT 52 CHECK (carcass_yield_percentage >= 0 AND carcass_yield_percentage <= 100),
  arroba_price numeric NOT NULL DEFAULT 0 CHECK (arroba_price >= 0),
  carcass_weight_kg numeric NOT NULL DEFAULT 0 CHECK (carcass_weight_kg >= 0),
  arrobas_per_head numeric NOT NULL DEFAULT 0 CHECK (arrobas_per_head >= 0),
  ...
  profit_per_head numeric NOT NULL DEFAULT 0,
  ...
  expense_percentage numeric NOT NULL DEFAULT 0,
