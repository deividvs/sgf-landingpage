...
export function generateAnnualResultsPDF(
  ...
  const perHeadMetrics = [
    ...
    ['Custo + Despesa Total:', formatCurrency(calculations.cost_plus_expense_per_head)]
  ];
  ...
  doc.rect(14, yPos - 6, pageWidth - 28, 30, 'F');
  ...
  doc.text(
    `${calculations.final_result >= 0 ? 'Lucro' : 'Prejuizo'}: ${formatCurrency(Math.abs(calculations.final_result))}`,
    14 + 5,
    yPos
  );
  ...
  doc.text(
    `Margem de Lucro: ${formatNumber(calculations.profit_margin_percentage, 2)}%`,
    14 + 5,
    yPos
  );
