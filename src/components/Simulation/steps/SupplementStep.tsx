...
export function SupplementStep({ data, onUpdate }: Props) {
  ...
  const supplementTypes = [
    { label: 'Proteinado (0,1%)', percentage: 0.1 },
    { label: 'Proteinado (0,2%)', percentage: 0.2 },
    { label: 'Proteico energético (0,3%)', percentage: 0.3 },
    { label: 'Proteico energético (0,4%)', percentage: 0.4 },
    { label: 'Proteico energético (0,5%)', percentage: 0.5 },
    { label: 'Concentrado engorda (1%)', percentage: 1.0 },
    { label: 'Concentrado engorda (1,5%)', percentage: 1.5 },
    { label: 'Concentrado engorda (2%)', percentage: 2.0 },
    { label: 'Personalizado', percentage: null }
  ];

  const handleSupplementTypeChange = (value: string) => {
    const selected = supplementTypes.find(t => t.label === value);
    if (selected && selected.percentage !== null) {
      onUpdate({ supplement_percentage: selected.percentage });
    }
  };

  const isCustom = !supplementTypes.some(t => t.percentage === data.supplement_percentage && t.percentage !== null);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select
            value={isCustom ? 'Personalizado' : supplementTypes.find(t => t.percentage === data.supplement_percentage)?.label || ''}
            onChange={(e) => handleSupplementTypeChange(e.target.value)}
